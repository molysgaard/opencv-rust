...
fn run_binding_generator(
    ...
    let clang = Arc::new(Mutex::new(clang::Clang::new().expect("Cannot initialize clang")));
    eprintln!("=== Clang: {}", clang::get_version());
    let gen = opencv_binding_generator::Generator::new(opencv_header_dir, &additional_include_dirs, &SRC_CPP_DIR, clang.clone());
    ...
    modules.iter().for_each(|module| {
        ...
        let join_handle = {
            ...
            let opencv_header_dir = Arc::clone(&opencv_header_dir);
            {
                ...
                let debug = true;
                eprintln!("MORTEN WAS HERE; RUNNING THISG");
                let res = opencv_binding_generator::binding_generator_as_library_function(&*opencv_header_dir, &*SRC_CPP_DIR, &*OUT_DIR, module, &*additional_include_dirs, cclang, debug);
                //if !res.success() {
...
pub fn gen_wrapper(
    ...
    run_binding_generator(modules, job_server, opencv_header_dir, opencv)?;
